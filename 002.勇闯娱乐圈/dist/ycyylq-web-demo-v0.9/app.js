const STORAGE_KEY = "ycylq-map-prototype-v4";
...
const candidateArtists = [
  {
    ...
    attrs: { vocal: 58, acting: 31, variety: 42, charm: 65, stamina: 52, creativity: 46 },
  ...
  },
  {
    ...
    attrs: { vocal: 28, acting: 66, variety: 36, charm: 49, stamina: 58, creativity: 44 },
  ...
  },
  {
    ...
    attrs: { vocal: 34, acting: 35, variety: 70, charm: 62, stamina: 46, creativity: 55 },
  ...
  },
  {
    ...
    attrs: { vocal: 54, acting: 26, variety: 30, charm: 43, stamina: 50, creativity: 68 },
  ...
  },
  {
    ...
    attrs: { vocal: 32, acting: 54, variety: 46, charm: 68, stamina: 48, creativity: 50 },
  ...
  },
  {
    ...
    attrs: { vocal: 62, acting: 38, variety: 60, charm: 78, stamina: 64, creativity: 44 },
    ...
    projects: [
      {
        ...
        resourceTier: "基础训练",
        desc: "提升唱功和创作手感。",
        delta: { vocal: 6, creativity: 2, stamina: -1, stress: 5, trust: 1, sensibility: 1 },
      ...
      },
      {
        ...
        resourceTier: "基础训练",
        desc: "提升演技、魅力和镜头稳定性。",
        delta: { acting: 5, charm: 2, stamina: -1, stress: 5, trust: 1, sensibility: 2, romance: 1 },
      ...
      },
      {
        ...
        resourceTier: "基础训练",
        desc: "提升体能，降低部分压力。",
        delta: { stamina: 6, stress: -3, charm: 1, discipline: 3, romance: -1 },
    ...
    projects: [
      {
        ...
        resourceTier: "小透明资源",
        desc: "提升综艺感，获得热度。",
        delta: { variety: 6, heat: 5, fans: 2, stress: 5, ambition: 2 },
      ...
      },
      {
        ...
        resourceTier: "低门槛曝光",
        desc: "快速拉热度，但口碑有波动。",
        delta: { variety: 2, heat: 10, fans: 4, reputation: -4, stress: 5, trust: -1, ambition: 3, discipline: -1 },
      ...
      },
      {
        ...
        delta: { variety: 5, heat: 18, fans: 8, reputation: -2, stress: 10, ambition: 4 },
...
const goalTemplates = [
  ...
  },
  {
    ...
    requirements: { variety: 60, charm: 56 },
...
function createInitialState() {
  return {
    ...
    goalChoices: clone(goalTemplates),
    activeGoal: null,
    showGoalModal: false,
    rankUpNotice: null,
    phoneTab: "hot",
...
function normalizeState(nextState) {
  ...
  nextState.goalChoices ||= clone(goalTemplates);
  if (!nextState.activeGoal && nextState.goalChoices.length && !nextState.goalChoices.some((goal) => goal.id === "lay-flat")) {
    nextState.goalChoices = clone(goalTemplates);
  }
...
function getCafeScore(artist) {
  ...
  return Math.round(artist.market.heat + artist.market.fans + artist.market.reputation + honorBonus);
}
...
function getDominantPath() {
  ...
  const scores = {
    acting: state.artist.attrs.acting + state.artist.attrs.charm * 0.4,
    music: state.artist.attrs.vocal + state.artist.attrs.creativity * 0.5,
    variety: state.artist.attrs.variety + state.artist.attrs.charm * 0.5,
...
function selectArtist(id) {
  ...
  state.goalChoices = clone(goalTemplates);
...
function shouldRejectByCafe(project, cafeTier) {
  ...
  const capitalBuffer = Math.min(0.22, (state.artist.profile?.capital || 0) / 360);
...
function resolveCafeSnub(project, cafeTier) {
  ...
  return `硬闯 ${project.name} 被挡回来了。${line} 消耗 ${energyLoss} 精力，压力上升，口碑和信任小幅受损。当前 L${cafeTier.level}，对方要的是 L${project.minCafe} 的牌面。`;
}
...
function runProject(projectId) {
  ...
  state.eventText = extraEvent || lifeEvent || project.result;
  state.logs.unshift({
    week: state.week,
    text: `${getLocation(state.locationId).name} - ${project.name}：${state.eventText}`
  });
...
function advanceWeek() {
  ...
  state.artist.attrs.stamina = clamp(state.artist.attrs.stamina + 3);
    ...
    <section class="phone-panel">
      <div class="phone-head">
        ...
        <div class="phone-tabs">
          ...
          <button class="${tab === "message" ? "active" : ""}" data-phone-tab="message">消息</button>
        </div>
      </div>
...
function renderGoalChoice(goal) {
  ...
  return `
    <article class="goal-choice">
      <div>
        <span>${pathLabels[goal.path]}</span>
        <h4>${goal.name}</h4>
        <p>${goal.desc}</p>
        <em>${req}</em>
...
function renderArtistSheet(artist) {
  ...
  return `
    <section class="artist-sheet">
      ...
      <div class="meter-list">
        ${Object.entries(artist.attrs).map(([key, value]) => renderMeter(key, value)).join("")}
      </div>
